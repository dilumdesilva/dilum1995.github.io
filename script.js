...
function render() {
  ...
  grid.innerHTML = filtered.map((item, i) => {
    ...
    const href = isArticle ? item.link : (hasRepo ? null : (item.pdf || '#'));
    ...
    if (hasRepo) {
      const linksHtml = `<div class="card-links">
        <a href="${item.repo}" target="_blank" rel="noopener" class="card-link-icon" aria-label="Code repository" title="Code">
        ...
        ${item.pdf ? `<a href="${item.pdf}" target="_blank" rel="noopener" class="card-link-icon" aria-label="Slides" title="Slides">
