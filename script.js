  ...
  grid.innerHTML = filtered.map((item, i) => {
    ...
    if (isTalk && item.pdf) {
      linksHtml = `<div class="card-links">
        ${item.repo ? `<a href="${item.repo}" target="_blank" rel="noopener" class="card-link-icon" aria-label="Code repository" title="Code" onclick="event.stopPropagation();event.preventDefault();window.open(this.href,'_blank')">
